...
export async function fetchMarketsBySeriesSlug({ seriesSlug, limit = 50 }: { seriesSlug: string; limit?: number }): Promise<GammaValue[]> {
  const url = new URL("/markets", CONFIG.gammaBaseUrl);
...
export async function fetchLiveEventsBySeriesId({ seriesId, limit = 20 }: { seriesId: string; limit?: number }): Promise<GammaValue[]> {
...
export async function fetchActiveMarkets({ limit = 200, offset = 0 }: { limit?: number; offset?: number } = {}): Promise<GammaValue[]> {
  const url = new URL("/markets", CONFIG.gammaBaseUrl);
  url.searchParams.set("active", "true");
  url.searchParams.set("closed", "false");
  url.searchParams.set("enableOrderBook", "true");
...
export function pickLatestLiveMarket(markets: GammaValue[], nowMs: number = Date.now()): GammaValue | null {
  ...
  if (live.length > 0) return live[0]!.m;

  const upcoming = enriched
    .filter((x) => nowMs < x.endMs)
    .sort((a, b) => a.endMs - b.endMs);

  return upcoming.length > 0 ? upcoming[0]!.m : null;
}
...
export function filterBtcUpDown15mMarkets(markets: GammaValue[], { seriesSlug, slugPrefix }: { seriesSlug?: string; slugPrefix?: string } = {}): GammaValue[] {
