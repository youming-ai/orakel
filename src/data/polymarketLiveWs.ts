...
export function startPolymarketChainlinkPriceStream({
  ...
  const connect = (): void => {
    ...
    ws.on("message", (data: WebSocket.RawData) => {
      ...
      const dataRec = decoded && typeof decoded === "object" && !Array.isArray(decoded) ? (decoded as JsonRecord) : null;
      ...
      const updatedAtMs = ts !== null ? Math.floor(Number(payload.timestamp) * 1000) : ua !== null ? Math.floor(Number(payload.updatedAt) * 1000) : null;
...
export function startMultiPolymarketPriceStream(symbols: string[] = [], { wsUrl = CONFIG.polymarket.liveDataWsUrl }: { wsUrl?: string } = {}): MultiPolymarketStreamHandle {
