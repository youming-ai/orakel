...
export function startBinanceTradeStream({ symbol = CONFIG.markets?.[0]?.binanceSymbol, onUpdate }: BinanceTradeStreamParams = {}): WsStreamHandle {
...
export function startMultiBinanceTradeStream(symbols: string[] = []): MultiBinanceWsStreamHandle {
  ...
  const connect = (): void => {
    ...
    ws.on("message", (data: WebSocket.RawData) => {
      try {
        const msg: unknown = JSON.parse(data.toString());
        const payload = msg && typeof msg === "object" && "data" in msg && msg.data && typeof msg.data === "object" ? msg.data : msg;
        const rawSymbol = payload && typeof payload === "object" && "s" in payload ? String(payload.s || "").toUpperCase() : "";
