import { PROBABILITY_DEFAULTS } from "../core/config.ts";
import type { ProbabilityConfig } from "../core/configTypes.ts";
...
interface ScoreDirectionInput {
	...
	probabilityConfig?: Partial<ProbabilityConfig>;
}
...
export function scoreDirection(inputs: ScoreDirectionInput): ScoreResult {
	...
	const cfg = { ...PROBABILITY_DEFAULTS, ...probabilityConfig };
	...
	if (price !== null && vwap !== null && vwap > 0) {
		...
		const vwapSignal = clamp(vwapPct / cfg.vwapDistanceSaturation, -1, 1); // saturates at ±0.2% distance
	...
	if (vwapSlope !== null) {
		const slopeSignal = clamp(vwapSlope / cfg.vwapSlopeSaturation, -1, 1); // saturates at slope ±0.5
	...
	if (rsi !== null && rsiSlope !== null) {
		const rsiDeviation = clamp((rsi - 50) / cfg.rsiDeviationScale, -1, 1); // RSI 75 → +1, RSI 25 → -1
	...
	if (heikenColor) {
		const haStrength = clamp(heikenCount / cfg.haStreakSaturation, 0, 1); // 4+ bars → full strength
		if (heikenColor === "green" && heikenCount >= cfg.haMinConsecutiveBars) up += 1 * haStrength;
		if (heikenColor === "red" && heikenCount >= cfg.haMinConsecutiveBars) down += 1 * haStrength;
	}
...
export function estimatePriceToBeatProbability(params: {
	...
	const { currentPrice, priceToBeat, remainingMinutes, volatility15m, probabilityConfig } = params;
	const cfg = { ...PROBABILITY_DEFAULTS, ...probabilityConfig };
	...
	const sigma = Math.max(volatility15m ?? 0, cfg.minVolatilityFloor) * Math.max(timeScale, 0.5);
	const z = clamp(distanceRatio / sigma, -8, 8);
	return clamp(1 / (1 + Math.exp(-z * cfg.ptbSigmoidScale)), 0, 1);
}
