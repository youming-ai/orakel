...
const clobCircuitBreaker = {
	failures: 0,
	openUntil: 0,
	maxFailures: CONFIG.infra.circuitBreakerMaxFailures,
	cooldownMs: CONFIG.infra.circuitBreakerCooldownMs,
	isOpen(): boolean {
...
export async function fetchMarketData(
	...
	try {
		const wsTick = streams.spot.getLast(market.spotSymbol);
		...
		const currentPrice = market.resolutionSource === "spot" ? spotPrice : (chainlink?.price ?? null);
		if (market.resolutionSource !== "spot") {
			const priceUpdatedAt = chainlink?.updatedAt ?? null;
			if (priceUpdatedAt !== null && Date.now() - priceUpdatedAt > CONFIG.infra.maxPriceAgeMs) {
