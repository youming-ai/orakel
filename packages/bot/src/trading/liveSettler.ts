...
export class LiveSettler {
	...
	start(): void {
		...
		this.timer = setInterval(() => void this.settle(), CONFIG.maintenance.settlerPollIntervalMs);
		void this.settle();
		log.info(`LiveSettler started (poll every ${CONFIG.maintenance.settlerPollIntervalMs}ms)`);
	}
