...
export const EDGE_DEFAULTS: EdgeConfig = {
	...
	vigThreshold: 1.08,
	phaseEarlyRatio: 0.66,
	phaseLateCutoff: 0.33,
	strengthStrongEdge: 0.2,
	strengthGoodEdge: 0.1,
};

export const PROBABILITY_DEFAULTS: ProbabilityConfig = {
	vwapDistanceSaturation: 0.002,
	vwapSlopeSaturation: 0.5,
	rsiDeviationScale: 25,
	haStreakSaturation: 4,
	haMinConsecutiveBars: 2,
	minVolatilityFloor: 0.003,
	ptbSigmoidScale: 1.6,
	volatilityLookback: 60,
};

export const REGIME_DEFAULTS: RegimeConfig = {
	lowVolumeRatio: 0.6,
	vwapProximityThreshold: 0.001,
	vwapCrossCountThreshold: 3,
};
...
export const EXECUTION_DEFAULTS: ExecutionConfig = {
	...
	maxOrderPrice: 0.98,
	confidentPrice: 0.95,
	confidentOpposite: 0.05,
};

export const MAINTENANCE_DEFAULTS: MaintenanceConfig = {
	maxTradesToKeep: 200,
	signalLogRetentionDays: 90,
	balanceSnapshotRetentionDays: 30,
	safeModeThreshold: 3,
	pruneIntervalMs: 3_600_000,
	accountPruneTradesCount: 500,
	orderPollIntervalMs: 5_000,
	orderPruneCutoffMs: 20 * 60_000,
	settlerPollIntervalMs: 15_000,
};

export const INFRA_DEFAULTS: InfraConfig = {
	spotWsBaseUrl: "wss://stream.bybit.com/v5/public/spot",
	...
	redeemerPriorityFeeGwei: 30,
	redeemerDefaultBaseFeeGwei: 200,
	redeemerTxWaitTimeoutMs: 60_000,
	circuitBreakerMaxFailures: 5,
	circuitBreakerCooldownMs: 60_000,
	maxPriceAgeMs: 60_000,
	klineHistoryLimit: 240,
};
...
const EdgeConfigSchema = z
	.object({
		arbitrageThreshold: z.coerce.number().min(0).max(1).optional(),
		vigThreshold: z.coerce.number().min(1).max(2).optional(),
		phaseEarlyRatio: z.coerce.number().min(0).max(1).optional(),
		phaseLateCutoff: z.coerce.number().min(0).max(1).optional(),
		strengthStrongEdge: z.coerce.number().min(0).max(1).optional(),
		strengthGoodEdge: z.coerce.number().min(0).max(1).optional(),
	})
	.partial()
	.transform((value) => ({ ...EDGE_DEFAULTS, ...value }));

const ProbabilityConfigSchema = z
	.object({
		vwapDistanceSaturation: z.coerce.number().min(0).optional(),
		vwapSlopeSaturation: z.coerce.number().min(0).optional(),
		rsiDeviationScale: z.coerce.number().min(1).optional(),
		haStreakSaturation: z.coerce.number().int().min(1).optional(),
		haMinConsecutiveBars: z.coerce.number().int().min(0).optional(),
		minVolatilityFloor: z.coerce.number().min(0).optional(),
		ptbSigmoidScale: z.coerce.number().min(0).optional(),
		volatilityLookback: z.coerce.number().int().min(1).optional(),
	})
	.partial()
	.transform((value) => ({ ...PROBABILITY_DEFAULTS, ...value }));

const RegimeConfigSchema = z
	.object({
		lowVolumeRatio: z.coerce.number().min(0).max(1).optional(),
		vwapProximityThreshold: z.coerce.number().min(0).optional(),
		vwapCrossCountThreshold: z.coerce.number().int().min(1).optional(),
	})
	.partial()
	.transform((value) => ({ ...REGIME_DEFAULTS, ...value }));

const ExecutionConfigSchema = z
	.object({
		minOrderPrice: z.coerce.number().min(0).max(1).optional(),
		maxOrderPrice: z.coerce.number().min(0).max(1).optional(),
		confidentPrice: z.coerce.number().min(0).max(1).optional(),
		confidentOpposite: z.coerce.number().min(0).max(1).optional(),
	})
	.partial()
	.transform((value) => ({ ...EXECUTION_DEFAULTS, ...value }));

const MaintenanceConfigSchema = z
	.object({
		maxTradesToKeep: z.coerce.number().int().min(1).optional(),
		signalLogRetentionDays: z.coerce.number().int().min(1).optional(),
		balanceSnapshotRetentionDays: z.coerce.number().int().min(1).optional(),
		safeModeThreshold: z.coerce.number().int().min(1).optional(),
		pruneIntervalMs: z.coerce.number().int().min(1_000).optional(),
		accountPruneTradesCount: z.coerce.number().int().min(1).optional(),
		orderPollIntervalMs: z.coerce.number().int().min(500).optional(),
		orderPruneCutoffMs: z.coerce.number().int().min(1_000).optional(),
		settlerPollIntervalMs: z.coerce.number().int().min(1_000).optional(),
	})
	.partial()
	.transform((value) => ({ ...MAINTENANCE_DEFAULTS, ...value }));

const InfraConfigSchema = z
	.object({
		spotWsBaseUrl: z.string().optional(),
		polymarketDataApiUrl: z.string().optional(),
		defaultFallbackRpcUrls: z.array(z.string()).optional(),
		redeemerPriorityFeeGwei: z.coerce.number().min(0).optional(),
		redeemerDefaultBaseFeeGwei: z.coerce.number().min(0).optional(),
		redeemerTxWaitTimeoutMs: z.coerce.number().int().min(1_000).optional(),
		circuitBreakerMaxFailures: z.coerce.number().int().min(1).optional(),
		circuitBreakerCooldownMs: z.coerce.number().int().min(1_000).optional(),
		maxPriceAgeMs: z.coerce.number().int().min(1_000).optional(),
		klineHistoryLimit: z.coerce.number().int().min(10).optional(),
	})
	.partial()
	.transform((value) => ({ ...INFRA_DEFAULTS, ...value }));
...
const ConfigFileSchema = z
	.object({
		...
		probability: ProbabilityConfigSchema.optional(),
	...
	.transform((value) => {
		...
		return {
			...
			perMarketStrategy,
			risk: value.risk,
			edge: EdgeConfigSchema.parse(value.edge ?? {}),
			probability: ProbabilityConfigSchema.parse(value.probability ?? {}),
			regime: RegimeConfigSchema.parse(value.regime ?? {}),
			execution: ExecutionConfigSchema.parse(value.execution ?? {}),
			maintenance: MaintenanceConfigSchema.parse(value.maintenance ?? {}),
			infra: InfraConfigSchema.parse(value.infra ?? {}),
		};
	});
...
const FILE_EDGE = FILE_CONFIG.edge;
const FILE_PROBABILITY = FILE_CONFIG.probability;
const FILE_REGIME = FILE_CONFIG.regime;
const FILE_EXECUTION = FILE_CONFIG.execution;
const FILE_MAINTENANCE = FILE_CONFIG.maintenance;
const FILE_INFRA = FILE_CONFIG.infra;
...
export function reloadConfig(): AppConfig {
	...
	CONFIG.edge = { ...fileConfig.edge };
	CONFIG.probability = { ...fileConfig.probability };
	CONFIG.regime = { ...fileConfig.regime };
	CONFIG.execution = { ...fileConfig.execution };
	CONFIG.maintenance = { ...fileConfig.maintenance };
	CONFIG.infra = { ...fileConfig.infra };
