...
function numberInput(value: number, onValue: (n: number) => void, step = 0.01, min = 0, max?: number) {
	return (
		<input
			type="number"
			...
			onChange={(e) => {
				...
				onValue(max !== undefined ? Math.min(max, Math.max(min, n)) : Math.max(min, n));
			}}
		/>
	);
}
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
						<div className="space-y-1">
							...
							<div className="font-mono text-xs space-y-0.5">
								<div>
									EARLY: <span className="text-emerald-400">{(strategyView.edgeThresholdEarly * 100).toFixed(1)}%</span>
								</div>
								<div>
									MID: <span className="text-amber-400">{(strategyView.edgeThresholdMid * 100).toFixed(1)}%</span>
								</div>
								<div>
									LATE: <span className="text-red-400">{(strategyView.edgeThresholdLate * 100).toFixed(1)}%</span>
								</div>
							</div>
						</div>
						<div className="space-y-1">
					...
					<div className="pt-2 border-t border-border">
						...
						<div className="flex flex-wrap gap-3 font-mono text-xs">
							<span>
								CHOP: <span className="text-amber-400">x{strategyView.regimeMultipliers.CHOP}</span>
							</span>
							<span>
								RANGE: <span className="text-muted-foreground">x{strategyView.regimeMultipliers.RANGE}</span>
							</span>
							<span>
								ALIGNED: <span className="text-emerald-400">x{strategyView.regimeMultipliers.TREND_ALIGNED}</span>
							</span>
							<span>
								OPPOSED: <span className="text-red-400">x{strategyView.regimeMultipliers.TREND_OPPOSED}</span>
							</span>
						</div>
					</div>
				</CardContent>
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<ParamField label="Edge EARLY">
							{numberInput(form.edgeThresholdEarly, (v) => setForm((s) => ({ ...s, edgeThresholdEarly: v })))}
						</ParamField>
						<ParamField label="Edge MID">
							{numberInput(form.edgeThresholdMid, (v) => setForm((s) => ({ ...s, edgeThresholdMid: v })))}
						</ParamField>
						<ParamField label="Edge LATE">
							{numberInput(form.edgeThresholdLate, (v) => setForm((s) => ({ ...s, edgeThresholdLate: v })))}
						</ParamField>
						<ParamField label="MinProb EARLY">
							{numberInput(form.minProbEarly, (v) => setForm((s) => ({ ...s, minProbEarly: v })))}
						</ParamField>
						<ParamField label="MinProb MID">
							{numberInput(form.minProbMid, (v) => setForm((s) => ({ ...s, minProbMid: v })))}
						</ParamField>
						<ParamField label="MinProb LATE">
							{numberInput(form.minProbLate, (v) => setForm((s) => ({ ...s, minProbLate: v })))}
						</ParamField>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<ParamField label="Blend Volatility">
							{numberInput(form.blendVol, (v) => setForm((s) => ({ ...s, blendVol: v })), 0.01, 0, 1)}
						</ParamField>
						<ParamField label="Blend Technical">
							{numberInput(form.blendTa, (v) => setForm((s) => ({ ...s, blendTa: v })), 0.01, 0, 1)}
						</ParamField>
						<div className="space-y-1">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<ParamField label="Max Trade USDC">
							{numberInput(form.maxTradeSizeUsdc, (v) => setForm((s) => ({ ...s, maxTradeSizeUsdc: v })), 0.1, 0)}
						</ParamField>
						<ParamField label="Max Positions">
							{numberInput(form.maxOpenPositions, (v) => setForm((s) => ({ ...s, maxOpenPositions: v })), 1, 0)}
						</ParamField>
						<ParamField label="Daily Loss USDC">
							{numberInput(form.dailyMaxLossUsdc, (v) => setForm((s) => ({ ...s, dailyMaxLossUsdc: v })), 0.1, 0)}
						</ParamField>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						<ParamField label="CHOP">
							{numberInput(form.regimeCHOP, (v) => setForm((s) => ({ ...s, regimeCHOP: v })))}
						</ParamField>
						<ParamField label="RANGE">
							{numberInput(form.regimeRANGE, (v) => setForm((s) => ({ ...s, regimeRANGE: v })))}
						</ParamField>
						<ParamField label="TREND_ALIGNED">
							{numberInput(form.regimeTREND_ALIGNED, (v) => setForm((s) => ({ ...s, regimeTREND_ALIGNED: v })))}
						</ParamField>
						<ParamField label="TREND_OPPOSED">
							{numberInput(form.regimeTREND_OPPOSED, (v) => setForm((s) => ({ ...s, regimeTREND_OPPOSED: v })))}
						</ParamField>
					</div>
