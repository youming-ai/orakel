...
function buildCumulativePnlData(trades: PaperTrade[]) {
...
function buildMarketBreakdown(trades: PaperTrade[]) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <StatCard
          ...
          color={stats.winRate >= 0.5 ? "text-emerald-400" : stats.winRate > 0 ? "text-red-400" : undefined}
        />
                ...
                <AreaChart data={pnlData}>
                  <defs>
                    <linearGradient id="pnlGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={COLORS.pnlPositive} stopOpacity={0.3} />
                      <stop offset="95%" stopColor={COLORS.pnlPositive} stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke={COLORS.grid} />
                  <XAxis dataKey="idx" tick={{ fontSize: 10, fill: COLORS.axis }} />
                  <YAxis tick={{ fontSize: 10, fill: COLORS.axis }} tickFormatter={(v: number) => `${v}`} />
                  <Tooltip
            ...
            {marketData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={marketData} barGap={2}>
                  <CartesianGrid strokeDasharray="3 3" stroke={COLORS.grid} />
                  <XAxis dataKey="market" tick={{ fontSize: 10, fill: COLORS.axis }} />
                  <YAxis tick={{ fontSize: 10, fill: COLORS.axis }} allowDecimals={false} />
                  <Tooltip
                    contentStyle={{ background: "#1a1a2e", border: "1px solid #333", borderRadius: 6, fontSize: 11 }}
                  />
                  <Bar dataKey="wins" stackId="a" fill={COLORS.win} radius={[0, 0, 0, 0]} name="Wins" />
                  <Bar dataKey="losses" stackId="a" fill={COLORS.loss} radius={[0, 0, 0, 0]} name="Losses" />
                  <Bar dataKey="pending" stackId="a" fill={COLORS.pending} radius={[2, 2, 0, 0]} name="Pending" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex items-center justify-center h-full text-xs text-muted-foreground">
      ...
      {stats.pending > 0 && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Badge variant="secondary" className="text-[10px] px-1.5 bg-amber-500/15 text-amber-400">
